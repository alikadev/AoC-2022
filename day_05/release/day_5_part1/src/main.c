...
stk_t *get_before_last_node(stk_t *stack)
{
	if(stack->n == NULL) return stack; 
	while(stack->n->n != NULL)
		stack = stack->n;
	return stack;
}

void transfer(int dest, int src)
{
	stk_t *srcBLast = get_before_last_node(&(roots[src]));
	stk_t *destLast = get_before_last_node(&(roots[dest]));
	if(destLast->n)
		destLast = destLast->n;
	destLast->n = srcBLast->n;
	srcBLast->n = NULL;
}
...
int write_data_output(void)
{
	...
	for (int i = 0; i < nbStack; ++i)
	{
		data[i] = get_before_last_node(&(roots[i]))->n->v;
	}
